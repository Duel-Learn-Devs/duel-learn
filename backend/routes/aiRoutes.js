...
// Test endpoint for the AI routes
...
router.post("/generate-summary", async (req, res) => {
  ...
  try {
    ...
    console.log("Extracted data:", { tags, items });
    ...
    const prompt = `Generate a concise 5-8 word summary based on these study materials:
Tags: ${tags ? tags.join(', ') : 'No tags'}
Items: ${items.map(item => `${item.term}: ${item.definition}`).join('\n')}

Rules:
1. Summary must be 5-8 words only
2. Make it descriptive but concise
3. Focus on the main theme or concept
